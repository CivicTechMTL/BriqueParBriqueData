...
CKAN_FACILITIES_ID = (
    "4731b64f-29cc-4e08-bc44-8752ae2fcafb"  # CSV French, confirmed 2026-05-03
)
...
CKAN_POI_ID = (
    "edce22aa-f7cc-495e-9c53-b367f68309f6"  # lieux d'interet (points of interest)
)
...
QPAREB_MEDIANS = {
    ...
    "Le Plateau-Mont-Royal": {
        "condo": 700000,
        "single_family": 1300000,
        "plex": 1220000,
        "multi": 2800000,
    },
    "Rosemont-La Petite-Patrie": {
        "condo": 595000,
        "single_family": 1155000,
        "plex": 1022000,
        "multi": 2200000,
    },
    "Cote-des-Neiges-Notre-Dame-de-Grace": {
        "condo": 606000,
        "single_family": 1340000,
        "plex": 1044000,
        "multi": 2400000,
    },
    "Ville-Marie": {
        "condo": 630000,
        "single_family": 1450000,
        "plex": 1100000,
        "multi": 3500000,
    },
    "Outremont": {
        "condo": 680000,
        "single_family": 1800000,
        "plex": 1400000,
        "multi": 3200000,
    },
    "Westmount": {
        "condo": 750000,
        "single_family": 2800000,
        "plex": 1800000,
        "multi": 4000000,
    },
    "Le Sud-Ouest": {
        "condo": 550000,
        "single_family": 900000,
        "plex": 870000,
        "multi": 1900000,
    },
    "Villeray-Saint-Michel-Parc-Extension": {
        "condo": 545000,
        "single_family": 711000,
        "plex": 901000,
        "multi": 1900000,
    },
    "Mercier-Hochelaga-Maisonneuve": {
        "condo": 430000,
        "single_family": 680000,
        "plex": 750000,
        "multi": 1600000,
    },
    "Verdun": {
        "condo": 500000,
        "single_family": 850000,
        "plex": 820000,
        "multi": 1800000,
    },
    "Ahuntsic-Cartierville": {
        "condo": 480000,
        "single_family": 750000,
        "plex": 790000,
        "multi": 1700000,
    },
    "Saint-Laurent": {
        "condo": 450000,
        "single_family": 700000,
        "plex": 730000,
        "multi": 1550000,
    },
    "Saint-Leonard": {
    ...
    "Riviere-des-Prairies-Pointe-aux-Trembles": {
        "condo": 430000,
        "single_family": 625000,
        "plex": 670000,
        "multi": 1400000,
    },
    "L-Ile-Bizard-Sainte-Genevieve": {
        "condo": 450000,
        "single_family": 780000,
        "plex": 750000,
        "multi": 1600000,
    },
    "Montreal-Nord": {
        "condo": 390000,
        "single_family": 560000,
        "plex": 620000,
        "multi": 1300000,
    },
    "Mont-Royal": {
        "condo": 680000,
        "single_family": 2200000,
        "plex": 1600000,
        "multi": 3500000,
    },
    "Beaconsfield": {
...
CUBF_LABELS = {
    1000: "Single-family",
    1001: "Single-family detached",
    1002: "Single-family semi-detached",
    1003: "Single-family row/townhouse",
    1010: "Duplex",
    1040: "Triplex",
    1090: "Plex (4-5 units)",
    1100: "Multi-residential (6+ units)",
    1210: "Seniors residence",
    2000: "Commercial",
    2010: "Retail",
    2020: "Office",
    3000: "Industrial",
...
MUNICIPALITE_MAP = {
    ...
    "01": "Ahuntsic-Cartierville",
    "02": "Anjou",
    "03": "Cote-des-Neiges-Notre-Dame-de-Grace",
    "04": "Lachine",
    "05": "LaSalle",
    "06": "Le Plateau-Mont-Royal",
    "07": "Le Sud-Ouest",
    "08": "L-Ile-Bizard-Sainte-Genevieve",
    "09": "Mercier-Hochelaga-Maisonneuve",
    "10": "Montreal-Nord",
    "11": "Outremont",
    "12": "Pierrefonds-Roxboro",
    "13": "Riviere-des-Prairies-Pointe-aux-Trembles",
...
def backfill_boroughs_from_permits():
    ...
    if not permits_path.exists():
        print("  Cannot backfill: permits CSV missing")
        return
    ...
    with open(permits_path, encoding="utf-8", errors="replace") as f:
        ...
        for row in reader:
            ...
            borough = (
                ...
                or row.get("NOM_ARROND")
                or row.get("ARRONDISSEMENT")
                or ""
            ).strip()
            ...
            if borough and street:
                sn = norm(street)
                if sn:
                    street_to_borough[sn] = borough
    if not street_to_borough:
        print("  No mappings found in permits")
        return
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, street_norm FROM properties WHERE borough IS NULL OR borough=''"
    )
    rows = c.fetchall()
    updated = 0
    for pid, sn in rows:
    ...
    conn.commit()
    c.execute(
        "SELECT borough,COUNT(*) FROM properties WHERE borough!='' GROUP BY borough ORDER BY 2 DESC LIMIT 5"
    )
    top = c.fetchall()
    conn.close()
...
def build_stm_db():
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        with zf.open("trips.txt") as f:
            reader = csv.DictReader(io.TextIOWrapper(f, encoding="utf-8"))
            for row in reader:
                trip_to_route[row.get("trip_id", "").strip()] = row.get(
                    "route_id", ""
                ).strip()
    ...
    conn.close()
    print(
        f"  ✓ STM: {len(stops):,} raw stops → {metro_count} metro stations (deduped) · {highfreq_count} high-freq bus"
    )
...
def load_stm_stops():
    ...
    print(
        f"  ✓ STM loaded: {len(STM_STOPS):,} stops ({metro} metro, {hfreq} high-freq bus)"
    )
...
def nearest_stm(lat, lng):
    ...
    return (
        best_metro[0],
        round(best_metro[1]) if best_metro[1] < float("inf") else None,
        best_bus[0],
        round(best_bus[1]) if best_bus[1] < float("inf") else None,
    )
...
def _nearest_in_store(store, grid, lat, lng, max_m=1000, filter_fn=None):
    ...
    if not store or not lat or not lng:
        return None, None
...
"""
    ...
    if batch:
        c.executemany(
            "INSERT INTO parks(name,lat,lng,area_m2,ptype) VALUES(?,?,?,?,?)", batch
        )
        conn.commit()
    conn.close()
...
def _build_parks_from_geojson_api_v2(c):
    ...
    for url in urls:
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "MTLPropIntel/2.0"}
            )
            with urllib.request.urlopen(req, timeout=45) as r:
                raw = json.loads(r.read())
            features = raw.get("features") or raw.get("result", {}).get("records") or []
            added = 0
            for feat in features:
                try:
                    props = feat.get("properties", feat)
                    name = (
                        props.get("Nom") or props.get("NOM") or ""
                    ).strip() or "Parc (sans nom)"
                    ...
                    if not lat or not lng:
                        lat, lng = extract_centroid(
                            feat.get("geometry") or props.get("geometry")
                        )
                    if not lat or not lng:
                        continue
                    if not (45.3 < lat < 45.8 and -74.1 < lng < -73.3):
                        continue
                    c.execute(
                        "INSERT INTO parks(name,lat,lng,area_m2,ptype) VALUES(?,?,?,?,?)",
...
def _build_parks_from_geojson_api():
    ...
    def centroid(coords):
        ...
        return (
            (lat, lng) if (45.3 < lat < 45.8 and -74.1 < lng < -73.3) else (None, None)
        )
    ...
    for feat in gj.get("features", []):
        try:
            ...
            name = (
                props.get("Nom") or props.get("NOM") or props.get("TOPONYME") or ""
            ).strip() or "Parc (sans nom)"
            area = safe_float(props.get("SUPERFICIE"))
...
def build_commercial_db():
    ...
    with open(csv_path, encoding="utf-8", errors="replace") as f:
        ...
        for row in reader:
            try:
                lat = safe_float(
                    row.get("LAT") or row.get("LATITUDE") or row.get("Latitude")
                )
                lng = safe_float(
                    row.get("LONG") or row.get("LONGITUDE") or row.get("Longitude")
                )
                if not lat or not lng:
                    continue
    ...
    if batch:
        c.executemany(
            "INSERT INTO commercial(name,lat,lng,usage1,usage2,vacant,arrond) VALUES(?,?,?,?,?,?,?)",
            batch,
        )
        conn.commit()
    conn.close()
        ...
        try:
            ...
            preferred = (
                [
                    ...
                    and "français" in r.get("name", "").lower()
                ]
                or [r for r in resources if r.get("format", "").upper() == "GEOJSON"]
                or [r for r in resources if r.get("datastore_active")]
                or [
                    r
                    for r in resources
...
def load_facilities_from_ckan():
    ...
    if not records:
        ...
        for rid_try in [
            ...
            if records:
                print(
                    f"  ✓ Facilities resource {rid_try} returned {len(records)} records"
                )
                break
    if not records:
        try:
    ...
    for rec in records:
        try:
            ...
            name = (
                rec.get("titre_lieu") or rec.get("nom") or rec.get("name") or ""
            ).strip()
            fac_type = (
                rec.get("types") or rec.get("categories") or rec.get("type") or ""
            ).strip()
            borough = (
                rec.get("arrondissements") or rec.get("arrondissement") or ""
            ).strip()
...
def build_geocache():
    ...
    if cache_path.exists() and (
        cache_path.stat().st_mtime > __import__("time").time() - 86400 * 30
    ):
        try:
            ...
            if len(ADDR_GEOCACHE) < 80000:
                print(
                    f"  Geocache: {len(ADDR_GEOCACHE):,} streets — too few, rebuilding…"
                )
                ADDR_GEOCACHE = {}
                ...
                print(
                    f"  ✓ Geocache (cache): {len(ADDR_GEOCACHE):,} streets, {total:,} addresses"
                )
                return
        except Exception:
            ADDR_GEOCACHE = {}
    ...
    if not geojson_path.exists():
        try:
            ...
            rs_url = (
                f"https://donnees.montreal.ca/api/3/action/resource_show?id={ADDR_RID}"
            )
            req = urllib.request.Request(
                rs_url, headers={"User-Agent": "MTLPropIntel/2.0"}
            )
            with urllib.request.urlopen(req, timeout=15) as r:
            ...
            req2 = urllib.request.Request(
                dl_url, headers={"User-Agent": "MTLPropIntel/2.0"}
            )
            with urllib.request.urlopen(req2, timeout=300) as r2, open(
                geojson_path, "wb"
            ) as fout:
                while True:
            ...
            try:
                ...
                for feat in parser:
                    try:
                        ...
                        civic_from = safe_int(
                            props.get("ADDR_DE") or props.get("addr_de")
                        )
                        generique = (
                            props.get("GENERIQUE") or props.get("generique") or ""
                        ).strip()
                        specifique = (
                            props.get("SPECIFIQUE") or props.get("specifique") or ""
                        ).strip()
...
def _build_geocache_from_datastore(cache_path):
    ...
    while True:
        try:
            url = (
                f"https://donnees.montreal.ca/api/3/action/datastore_search"
...
def geocode_address(civic_num, street_raw):
    ...
    """
    if not GEOCACHE_LOADED or not street_raw:
        return None, None
    ...
    words = [
        ...
        if len(w) >= 5
        and w
        not in {"saint", "sainte", "avenue", "boulevard", "chemin", "route", "place"}
    ]
    if words:
...
def get_amenity_proximity(lat, lng):
    ...
    # Parks
    if PARKS_STORE:
        near_park, park_dist = _nearest_in_store(
            PARKS_STORE, _PARK_GRID, lat, lng, 1000
        )
    ...
    if FACILITIES_STORE:
        ...
        def is_library(f):
            return (
                "biblioth" in (f.get("installations", "") + f.get("types", "")).lower()
            )

        def is_pool(f):
            return (
                "piscine" in (f.get("installations", "") + f.get("types", "")).lower()
            )

        def is_arena(f):
            return (
                "ar" in (f.get("installations", "") + f.get("types", "")).lower()
                and "na" in (f.get("installations", "") + f.get("types", "")).lower()
            )

        def is_community(f):
            return (
                "communaut" in (f.get("types", "") + f.get("installations", "")).lower()
            )
        ...
        result["facilities_1km"] = _count_within(
            FACILITIES_STORE, _FAC_GRID, lat, lng, 1000
        )

        lib, lib_d = _nearest_in_store(
            FACILITIES_STORE, _FAC_GRID, lat, lng, 2000, is_library
        )
        result["nearest_library"] = lib["name"] if lib else None
        result["nearest_library_dist"] = lib_d

        pool, pool_d = _nearest_in_store(
            FACILITIES_STORE, _FAC_GRID, lat, lng, 2000, is_pool
        )
    ...
    if POI_STORE:
        ...
        def is_cultural(f):
            return f.get("famille", "").lower() in ("culturel", "éducation")

        def is_sport(f):
            return f.get("famille", "").lower() in ("sports et loisirs", "sport")

        result["poi_500m"] = _count_within(POI_STORE, _POI_GRID, lat, lng, 500)
        result["cultural_poi_1km"] = _count_within(
            POI_STORE, _POI_GRID, lat, lng, 1000, is_cultural
        )
        result["sport_poi_1km"] = _count_within(
            POI_STORE, _POI_GRID, lat, lng, 1000, is_sport
        )
    ...
    if COMMERCIAL_STORE:
        ...
        def is_vacant(f):
            return f.get("vacant", 0) == 1

        result["commercial_200m"] = _count_within(
            COMMERCIAL_STORE, _COM_GRID, lat, lng, 200
        )
        result["commercial_500m"] = _count_within(
            COMMERCIAL_STORE, _COM_GRID, lat, lng, 500
        )
        result["vacant_commercial_200m"] = _count_within(
            COMMERCIAL_STORE, _COM_GRID, lat, lng, 200, is_vacant
        )
...
def cubf_label(code):
    try:
        c = int(code)
        if c in CUBF_LABELS:
            return CUBF_LABELS[c]
        d = (c // 10) * 10
        if d in CUBF_LABELS:
            return CUBF_LABELS[d]
        h = (c // 100) * 100
        if h in CUBF_LABELS:
            return CUBF_LABELS[h]
        return f"Code {code}"
    except:
...
def safe_int(v):
    try:
        return (
            int(str(v).strip()) if v and str(v).strip() not in ("", "0", "-1") else None
        )
    except:
        return None
...
def safe_float(v):
    try:
        return float(str(v).strip()) if v and str(v).strip() not in ("", "-1") else None
    except:
        return None
...
def build_properties_db():
    ...
    with open(csv_path, encoding="utf-8", errors="replace") as f:
        ...
        for row in reader:
            try:
                ...
                borough = extract_borough_from_rue(
                    _rue_raw, _rem_code, _mun_raw, _civic_num
                )
    ...
    c.execute(
        "SELECT borough, COUNT(*) FROM properties WHERE borough!='' GROUP BY borough ORDER BY 2 DESC LIMIT 20"
    )
    ...
    with open(csv_path, encoding="utf-8", errors="replace") as f:
        ...
        for row in reader:
            try:
                ...
                m = re.match(r"^\s*(\d+)\s+(.+)$", emplacement)
                civic_num = (
                    safe_int(m.group(1)) if m else safe_int(row.get("NO_CIVIQUE"))
                )
                street_raw = (
                    m.group(2).strip()
                    if m
                    else (row.get("NOM_RUE") or emplacement).strip()
                )
                arrond = (
                    row.get("arrondissement") or row.get("NOM_ARROND") or ""
                ).strip()
                cat = (
                    row.get("description_type_demande") or row.get("_CATEGORIE") or ""
                ).strip()
                desc = (row.get("nature_travaux") or row.get("_TRAVAUX") or "").strip()
                # Use date_emission as the delivery date
                date_val = (
                    row.get("date_emission") or row.get("DATE_DELIVRANCE") or ""
                ).strip()
                ...
                if len(batch) >= 5000:
                    c.executemany(
                        "INSERT INTO permits(no_permis,date_deliv,arrond,norm_arrond,civic,street,street_norm,categorie,nature_travaux,description,valeur) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                        batch,
                    )
                    conn.commit()
                    batch = []
...
def load_contaminated():
    ...
    if not path.exists():
        try:
            download(
                SOURCES["contaminated"]["url"], path, SOURCES["contaminated"]["desc"]
            )
        except Exception as e:
            print(f"  Contaminated unavailable: {e}")
            return []
    try:
        with open(path) as f:
            data = json.load(f)
...
def build_market_rates():
        ...
        try:
            ...
            for row in rows_in_mem:
                ...
                try:
                    ...
                    arrond = (
                        row.get("arrondissement") or row.get("ARRONDISSEMENT") or ""
                    ).strip()
                    ...
                    categorie = (
                        (
                            ...
                            or row.get("description_type_batiment")
                            or ""
                        )
                        .strip()
                        .lower()
                    ...
                    if any(
                        w in categorie
                        for w in ["résidentiel", "residential", "logement"]
                    ):
                        if any(
                            w in categorie for w in ["multi", "appartement", "immeuble"]
                        ):
                            grp = "multi"
                        ...
                        w in categorie for w in ["commercial", "bureau", "industriel"]
                    ):
                        grp = "commercial"
                    else:
                        grp = "other"
    ...
    try:
        ...
        """)
        avg_dims = {
            r[0]: {"avg_lot": r[1] or 200, "avg_bldg": r[2] or 120, "n": r[3]}
            for r in c.fetchall()
        }
        conn.close()
    except Exception:
    ...
    for (arrond, grp), amounts in bucket.items():
        if len(amounts) < 3:
            continue  # need at least 3 sales
        med = median(amounts)
        ...
        land_rate = max(
            400, (med - bldg_cost_est * dims["avg_bldg"]) / max(dims["avg_lot"], 50)
        )

        if arrond not in rates:
            rates[arrond] = {}
        rates[arrond][grp] = {
    ...
    for (arrond, grp), amounts in bucket.items():
        if len(amounts) < 10:
            continue  # need decent sample before overriding QPAREB
        med = median(amounts)
    ...
    print(
        f"    Supplemented {supplemented} QPAREB median buckets with transaction data"
    )
...
def estimate_value(borough, r):
    ...
    if yr >= 2015:
        ...
        age_f = 0.32  # pre-1940 — land dominates value
    ...
    is_condo = (
        (cubf and int(str(cubf)[:2]) == 91)
        or categorie in ("CON", "CONDOMINIUM")
        or (units <= 1 and lot <= 80 and lot > 0)
    )
    if is_condo:
        qpareb_type = "condo"

    borough_medians = QPAREB_MEDIANS.get(b) or QPAREB_MEDIANS.get("_island")
    median_price = borough_medians.get(qpareb_type) or borough_medians.get(
        "single_family"
    )
    ...
    CONDO_TYPICAL_BY_BOROUGH = {
        "Ville-Marie": 65,
        "Le Plateau-Mont-Royal": 72,
        "Le Sud-Ouest": 70,
        "Rosemont-La Petite-Patrie": 72,
        "Villeray-Saint-Michel-Parc-Extension": 70,
        "Cote-des-Neiges-Notre-Dame-de-Grace": 75,
        "Outremont": 80,
        "Westmount": 85,
        "Mont-Royal": 85,
        "Pierrefonds-Roxboro": 95,
        "Ahuntsic-Cartierville": 80,
        "Saint-Laurent": 82,
        "Mercier-Hochelaga-Maisonneuve": 68,
        "Verdun": 70,
        "LaSalle": 80,
        "Lachine": 75,
    }
    ...
    if lot > 0 and typ["lot"] > 0:
        implied_land_total = median_price - bldg_replacement * (
            typ["bldg"] / max(bldg, 1)
        )
    ...
    if has_qpareb:
        parts.append(
            f"QPAREB/Centris Q4 2025 {qpareb_type} median for {b}: ${median_price:,} (size-adjusted ×{size_adj:.2f})"
        )
    else:
        parts.append(
            f"No borough-specific QPAREB data — using island-wide median: ${median_price:,}"
        )
    if method_b:
        parts.append(
            f"Income: {units} units × ${rent_mo}/mo CMHC 2024 rent × 12 × 0.65 NOI ÷ {cap}% cap = ${round(method_b/1000)*1000:,}"
        )
    parts.append(f"Cost: {lot}m² lot + {bldg}m² building × {age_f:.0%} age factor")
    parts.append(
        "Cross-reference with Evalweb (official assessed value) for your specific unit."
    )
    ...
    try:
        ...
        if civic_start and arrond_norm:
            ...
            if n > 0:
                return {
                    "total": n,
                    "demolitions": row[1] or 0,
                    "recent_construction": row[2] or 0,
                    "scope": "civic+borough",
                }
...
def enrich(r, ps, conn=None):
    ...
    metro_name, metro_dist, hfreq_bus_name, hfreq_bus_dist = nearest_stm(
        prop_lat, prop_lng
    )
    ...
    signals = {
        ...
        "nearpark": amenity.get("nearest_park_dist") is not None
        and amenity.get("nearest_park_dist", 9999) <= 300,
    ...
    return {
        ...
        "street": r.get("street"),
        "borough": borough,
        "use_type": r.get("cubf_label"),
        "cubf_code": r.get("cubf_code"),
        "year_built": r.get("year_built"),
        "nb_logements": r.get("nb_logements"),
        "nb_etages": r.get("nb_etages"),
        "superficie_terrain_m2": r.get("lot_m2"),
        "superficie_batiment_m2": r.get("bldg_m2"),
        ...
        "geocoded": geocoded,
        # Transit
        "nearest_rem": rem_name,
        "nearest_rem_dist_m": rem_dist,
        "within_800m_rem": rem_dist is not None and rem_dist <= 800,
        "nearest_metro": metro_name,
        "nearest_metro_dist_m": metro_dist,
        "within_500m_metro": metro_dist is not None and metro_dist <= 500,
        "nearest_hfreq_bus": hfreq_bus_name,
        "nearest_hfreq_bus_dist_m": hfreq_bus_dist,
...
def search_properties(q, limit=10):
    ...
    for r in rows:
        ...
        ps = permit_summary(
            conn, r.get("street_norm"), r.get("borough"), r.get("civic_start")
        )
        ...
        if civic:
            c.execute(
                """SELECT no_permis,date_deliv,categorie,description,valeur FROM permits
    ...
    def do_GET(self):
        ...
        if parsed.path == "/health":
                ...
                self.send_json(
                    {
                        "properties": props,
                        "permits": perms,
                        "contaminated_sites": len(CONTAMINATED),
                        "rem_stations": len(REM_STATIONS),
                    }
                )
            except Exception as e:
            ...
            try:
                ...
                wheres, args = [], []
                if borough:
                    wheres.append("borough=?")
                    args.append(borough)
                if cubf == "residential":
                    wheres.append("cubf_code>=1000 AND cubf_code<1010")
                elif cubf == "income":
                    wheres.append(
                        "(nb_logements>=2 OR (cubf_code>=1010 AND cubf_code<2000))"
                    )
                elif cubf == "plex":
                    wheres.append("nb_logements>=4")
                elif cubf == "commercial":
                    wheres.append("cubf_code>=2000 AND cubf_code<3000")
                if units_min > 1:
                    wheres.append("nb_logements>=?")
                    args.append(units_min)
                if floors_min > 1:
                    wheres.append("nb_etages>=?")
                    args.append(floors_min)
                if lot_min > 0:
                    wheres.append("lot_m2>=?")
                    args.append(lot_min)
                if bldg_min > 0:
                    wheres.append("bldg_m2>=?")
                    args.append(bldg_min)
                if year_min > 0:
                    wheres.append("year_built>=?")
                ...
                c.execute(
                    f"SELECT * FROM properties {wsql} ORDER BY {osql} LIMIT ? OFFSET ?",
                ...
                for r in rows:
                    ...
                    ps = (
                        permit_summary(conn, sn, r.get("borough"), r.get("civic_start"))
                        if sn
                        else {}
                    )
            ...
            try:
                ...
                metro = c.fetchone()[0]
                c.execute(
                    "SELECT COUNT(*) FROM stm_stops WHERE is_highfreq=1 AND is_metro=0"
                )
                hfreq = c.fetchone()[0]
                c.execute(
                    "SELECT * FROM stm_stops WHERE is_metro=1 ORDER BY trip_count DESC LIMIT 5"
                )
                sample_metro = [
                    dict(zip([d[0] for d in c.description], r)) for r in c.fetchall()
                ]
                c.execute(
                    "SELECT * FROM stm_stops WHERE is_highfreq=1 AND is_metro=0 ORDER BY trip_count DESC LIMIT 5"
                )
                sample_hfreq = [
                    dict(zip([d[0] for d in c.description], r)) for r in c.fetchall()
                ]
                ...
                self.send_json(
                    {
                        "total_stops": total,
                        "metro_stops": metro,
                        "highfreq_bus_stops": hfreq,
                        "stm_loaded": STM_LOADED,
                        "sample_metro": sample_metro,
                        "sample_highfreq_bus": sample_hfreq,
                    }
                )
            except Exception as e:
                self.send_json({"error": str(e)}, 500)
            ...
            sample = {}
            if MARKET_RATES:
                for borough, rates in list(MARKET_RATES.get("by_borough", {}).items())[
                    :8
                ]:
            ...
            try:
                ...
                key_words = [
                    w for w in sn.split() if len(w) >= 4 and w not in SKIP
                ] or sn.split()
                ...
                # Exact civic
                c.execute(
                    "SELECT id,civic_start,civic_end,street,street_norm,borough,full_address FROM properties WHERE civic_start <= ? AND COALESCE(civic_end, civic_start) >= ? AND street_norm LIKE ?",
                ...
                if civic:
                    c.execute(
                        "SELECT id,civic_start,civic_end,street,street_norm,borough,full_address FROM properties WHERE street_norm LIKE ? AND civic_start BETWEEN ? AND ? ORDER BY civic_start",
                        (
                            f"%{key_words[0] if key_words else sn}%",
                            max(0, civic - 50),
                            civic + 50,
                        ),
                    )
                ...
                if key_words:
                    ...
                    c.execute(
                        f"SELECT COUNT(*) FROM properties WHERE {like}",
                        [f"%{w}%" for w in key_words],
                    )
                    out["street_total"] = c.fetchone()[0]
                    c.execute(
                        f"SELECT DISTINCT street, street_norm, borough FROM properties WHERE {like} LIMIT 5",
                        [f"%{w}%" for w in key_words],
                    )
            ...
            try:
                ...
                c2 = conn2.cursor()
                c2.execute(
                    "SELECT borough,COUNT(*) FROM properties GROUP BY borough ORDER BY 2 DESC LIMIT 30"
                )
                out["db_boroughs"] = [
                    {"borough": r[0], "count": r[1]} for r in c2.fetchall()
                ]
...
def main():
    ...
    if not needs_rebuild:
        try:
            ...
            if (
                "borough" not in cols
                or "street_norm" not in cols
                or "lot_m2" not in cols
            ):
                ...
                c.execute(
                    "SELECT borough, COUNT(*) FROM properties WHERE borough!='' GROUP BY borough ORDER BY 2 DESC LIMIT 1"
                )
                top = c.fetchone()
                if (
                    top
                    and top[0] in ("Montreal", "Ville de Montreal", "montreal")
                    and top[1] > 50000
                ):
                    print(
                        f"  ⚠  Borough data corrupt: {top[1]:,} properties mapped to '{top[0]}'."
                    )
                    print(
                        f"  ⚠  Auto-deleting and rebuilding database — please wait ~3 minutes…"
                    )
                    conn.close()
                    DB_PATH.unlink()
                    needs_rebuild = True
                    ...
                    c.execute(
                        "SELECT COUNT(*) FROM properties WHERE street_norm LIKE '%(%)%'"
                    )
                    qual_count = c.fetchone()[0]
                    if qual_count > 0:
                        print(
                            f"  Fixing {qual_count:,} street_norm values with qualifiers…"
                        )
                        c.execute(
                            "SELECT id, street_norm FROM properties WHERE street_norm LIKE '%(%)%'"
                        )
                        stale = c.fetchall()
                        batch = [
                            (
                        ...
                        c.executemany(
                            "UPDATE properties SET street_norm=? WHERE id=?", batch
                        )
                        conn.commit()
    ...
    try:
        ...
        if empty_boroughs > 0:
            print(
                f"  {empty_boroughs:,} properties missing borough name — backfilling from permits…"
            )
    ...
    try:
        ...
        cc = conn_c.cursor()
        cc.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='commercial'"
        )
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='stm_stops'"
        )
        ...
        if has_stm:
            ...
            if has_stm:
                ...
                if metro_n > 80:  # old data — rebuild
                    print(
                        f"  STM: found {metro_n} metro entries (non-deduped) — rebuilding…"
                    )
                    has_stm = False
        conn.close()
        if not has_stm:
    ...
    def _bg():
        ...
        try:
            ...
            traceback.print_exc()
        try:
            load_facilities_from_ckan()
        except Exception as e:
            print(f"  [bg] Facilities: {e}")
        try:
            load_poi_from_ckan()
        except Exception as e:
            print(f"  [bg] POI: {e}")
